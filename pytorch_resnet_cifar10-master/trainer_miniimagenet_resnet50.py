...
def main():
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # 調整成 224x224，如果你要保持 84x84 請改掉這行
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406],  # ImageNet 預訓練模型的 normalize
                            std=[0.229, 0.224, 0.225])
    ])

    # 載入資料
    train_dataset = datasets.ImageFolder(root='/ssd5/Roy/mini-imagenet-tools/processed_images/train', transform=transform)
    val_dataset = datasets.ImageFolder(root='/ssd5/Roy/mini-imagenet-tools/processed_images/val', transform=transform)
    test_dataset = datasets.ImageFolder(root='/ssd5/Roy/mini-imagenet-tools/processed_images/test', transform=transform)
    ...
    if args.evaluate:
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet50_miniimagenet/MW/best_model.th"
        ...
        if args.pruning_method == 'KS':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet50_miniimagenet/KS/best_model.th"
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        # for i, batch in enumerate(val_loader):
            ...
            # print("Targets:", target.shape, target.dtype)
            # print("Predicted top-1:", output.argmax(dim=1)[:5])
            # print("True labels:", target[:5])
