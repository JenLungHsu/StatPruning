...
def main():
    ...
    if args.pruning_method:
        ...
        path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet152_tinyimagenet/best_model.th"
    ...
    # 載入資料
    train_dataset = datasets.ImageFolder(root='/ssd5/Roy/tiny-imagenet-200/train', transform=transform)
    val_dataset = datasets.ImageFolder(root='/ssd5/Roy/tiny-imagenet-200/val', transform=transform)
    test_dataset = datasets.ImageFolder(root='/ssd5/Roy/tiny-imagenet-200/test', transform=transform)
    ...
    if args.evaluate:
        ...
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet152_tinyimagenet/MW/best_model.th"
            checkpoint = torch.load(path, map_location='cuda') 
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
