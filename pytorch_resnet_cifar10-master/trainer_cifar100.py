...
def main():
    ...
    model = torch.nn.DataParallel(resnet_cifar100.__dict__[args.arch]())
    ...
    if args.pruning_method:
        ...
        if args.pruning_method == 'EMD0.05':
            ...
            # ---------32----------
            for i in range(4, 0, -1):  # 逆向刪除，避免索引錯亂
                del model.layer3[i]
    ...
    if args.evaluate:
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/MW/best_model.th"
        ...
        if args.pruning_method == 'KS':
            ...
            # path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100_sub/KS/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.05':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/EMD0.05/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.075':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/EMD0.075/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.1':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/EMD0.1/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'RedCircle':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/RedCircle/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.025':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/EMD0.025/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
