...
def main():
    ...
    if args.pruning_method:
        ...
        if args.pruning_method == 'MW':
            ...
            del model.module.layer3[16]
            for i in range(14, 10, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer3[i]
            del model.module.layer3[9]
            for i in range(6, 1, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer3[i]        
            
            del model.module.layer2[17]
            for i in range(15, 5, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer2[i]   
            del model.module.layer2[4]

            del model.module.layer1[6]

            print(model.module)

        elif args.pruning_method == 'KS_seed5':
            ...
            for i in range(16, 5, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer2[i]   
    ...
    if args.evaluate:
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110/MW/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'KS':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110/KS/best_model.th"
            # path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110_KS_e300/KS/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'KS_seed5':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110_KS_seed5/KS_seed5/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
