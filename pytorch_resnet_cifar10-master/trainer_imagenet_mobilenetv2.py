...
def main():
    ...
    if args.evaluate:
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_mobilenetv2_imagenet/MW/best_model.th"
        ...
        if args.pruning_method == 'KS':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_mobilenetv2_imagenet/KS/best_model.th"
            # path = "/ssd5/Roy/SR-init-main/SRinit/finetuned_model/ResNet50/imagenet_dali/1/scores.pt"
            checkpoint = torch.load(path, map_location='cuda')
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            # state_dict = torch.load(path, map_location='cuda')
