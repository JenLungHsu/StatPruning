...
def main():
    ...
    if args.evaluate:
        if args.pretrained:
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet18_tinyimagenet/best_model.th"
            checkpoint = torch.load(path, map_location='cuda') 
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet18_tinyimagenet/MW/best_model.th"
            checkpoint = torch.load(path, map_location='cuda') 
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
