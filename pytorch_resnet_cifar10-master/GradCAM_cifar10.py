...
model_ori = resnet_cifar10.__dict__['resnet56']()
ckpt = torch.load("/ssd5/Roy/pytorch_resnet_cifar10-master/pretrained_models/resnet56.th", weights_only=True, map_location=device)
state_dict = ckpt["state_dict"]
new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
model_ori.load_state_dict(new_state_dict)
model_ori.cuda()

model_MW = resnet_cifar10.__dict__['resnet56_MW']()
ckpt = torch.load("/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/MW/best_model.th", weights_only=True, map_location=device)
state_dict = ckpt["state_dict"]
new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
...
plt.imshow(rgb_img)
plt.axis('off')
plt.savefig("gradcam_cifar10_2.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_ori)
plt.axis('off')
plt.savefig("gradcam_cifar10_ori_2.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_MW)
plt.axis('off')
plt.savefig("gradcam_cifar10_MW_2.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")
