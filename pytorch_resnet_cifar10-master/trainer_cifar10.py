...
def main():
    ...
    if args.pretrained:
        path=os.path.join('/ssd5/Roy/pytorch_resnet_cifar10-master/pretrained_models', args.arch + '.th')
    ...
    if args.pruning_method:
        ...
        elif args.pruning_method == 'EMD0.05':
            # ---------224---------
            # del model.module.layer3[7]
            # del model.module.layer3[2]
            # del model.module.layer3[1]
            # for i in range(8, 0, -1):  # 逆向刪除，避免索引錯亂
            #     del model.module.layer2[i]
            # ---------32----------
            del model.module.layer3[7]
        ...
        elif args.pruning_method == 'EMD0.025':
            del model.module.layer3[2]
        ...
        elif args.pruning_method == 'EMD0.025_dr':
            del model.module.layer3[7]
            for i in range(8, 5, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer2[i]

        elif args.pruning_method == 'EMD0.05_dr':
            del model.module.layer3[7]
            del model.module.layer3[1]
            for i in range(8, 1, -1):  # 逆向刪除，避免索引錯亂
                del model.module.layer2[i]
    ...
    if args.evaluate:
        ...
        if args.pruning_method == 'EMD0.05':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.05/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.075':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.075/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.1':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.1/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'RedCircle':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/RedCircle/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.025':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.025/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)

        if args.pruning_method == 'EMD0.025_dr':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.025_dr/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
            
        if args.pruning_method == 'EMD0.05_dr':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56/EMD0.05_dr/best_model.th"
            checkpoint = torch.load(path)
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            model.load_state_dict(state_dict)
