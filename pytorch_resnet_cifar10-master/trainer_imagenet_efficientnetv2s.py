...
def main():
    ...
    if args.pruning_method:
        ...
        if args.pruning_method == 'MW':
            for i in range(14, 0, -1):  # 逆向刪除，避免索引錯亂
                del model.features[6][i]
            for i in range(8, 1, -1):  # 逆向刪除，避免索引錯亂
                del model.features[5][i]
            for i in range(5, 0, -1):  # 逆向刪除，避免索引錯亂
                del model.features[4][i]
            for i in range(3, 0, -1):  # 逆向刪除，避免索引錯亂
                del model.features[3][i]

            del model.features[2][3]
            del model.features[2][2]
    ...
    if args.evaluate:
        if args.pruning_method == 'MW':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_efficientnetv2s_imagenet/MW/best_model.th"
        ...
        if args.pruning_method == 'KS':
            path = "/ssd5/Roy/pytorch_resnet_cifar10-master/save_efficientnetv2s_imagenet/KS/best_model.th"
            # path = "/ssd5/Roy/SR-init-main/SRinit/finetuned_model/ResNet50/imagenet_dali/1/scores.pt"
            checkpoint = torch.load(path, map_location='cuda')
            state_dict = checkpoint['state_dict']  # 如果你的檔案裡有 `state_dict`，取出來
            # state_dict = torch.load(path, map_location='cuda')
