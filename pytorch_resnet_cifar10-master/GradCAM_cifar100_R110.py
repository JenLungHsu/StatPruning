...
model_ori = resnet_cifar100.__dict__['resnet110']()
ckpt = torch.load('/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110_cifar100/best_model.th', weights_only=True, map_location=device)
state_dict = ckpt["state_dict"]
new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
model_ori.load_state_dict(new_state_dict)

model_MW = resnet_cifar100.__dict__['resnet110_MW']()
ckpt = torch.load("/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet110_cifar100/MW/best_model.th", weights_only=True, map_location=device)
state_dict = ckpt["state_dict"]
new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
...
print(f"🖼️ 隨機選取 CIFAR-100 圖片索引: {idx}")
print(f"🏷️ 圖片 Ground Truth 類別: {label} ({classes[label]})")

# 反 normalize + to numpy for overlay
inv_transform = transforms.Normalize(
    mean=[-0.49139968/0.24703233, -0.48215827/0.24348505, -0.44653124/0.26158768],
...
plt.imshow(rgb_img)
plt.axis('off')
plt.savefig("gradcam_cifar100_R110.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_ori)
plt.axis('off')
plt.savefig("gradcam_cifar100_R110_ori.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_MW)
plt.axis('off')
plt.savefig("gradcam_cifar100_R110_MW.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")
