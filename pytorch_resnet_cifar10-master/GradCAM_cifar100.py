...
model_ori = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar100_resnet56", pretrained=True)
model_ori.cuda()

model_MW = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar100_resnet56", pretrained=False)
for i in range(4, 1, -1):  # 逆向刪除，避免索引錯亂
    del model_MW.layer3[i]
ckpt = torch.load("/ssd5/Roy/pytorch_resnet_cifar10-master/save_resnet56_cifar100/MW/best_model.th", weights_only=True, map_location=device)
...
plt.imshow(rgb_img)
plt.axis('off')
plt.savefig("gradcam_cifar100_8.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_ori)
plt.axis('off')
plt.savefig("gradcam_cifar100_ori_8.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")

plt.imshow(visualization_MW)
plt.axis('off')
plt.savefig("gradcam_cifar100_MW_8.png", bbox_inches='tight', pad_inches=0)
print("✔️ Grad-CAM 圖片已儲存為 gradcam_result.png")
