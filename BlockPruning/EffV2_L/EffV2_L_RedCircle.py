...
def EffV2_L_RedCircle():
    ...
    for i in range(24, 0, -1):  # 逆向刪除，避免索引錯亂
        del model.blocks[5][i]

    for i in range(18, 0, -1):  # 逆向刪除，避免索引錯亂
        del model.blocks[4][i]

    for i in range(9, 0, -1):  # 逆向刪除，避免索引錯亂
...
if __name__ == "__main__":
    ...
    model = EffV2_L_RedCircle().to(device)
    print(model)
