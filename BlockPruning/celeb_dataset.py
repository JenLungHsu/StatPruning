...
class CelebDFDataset(Dataset):
    ...
    def parse_datasets(self):
        file_path = "/ssd2/DeepFakes_may/celeb-df/" + self.split + ".txt"
    ...
    def get_label_distribution(self):
        ...
        labels = [1 if item["is_real"] else 0 for item in self.items]
...
if __name__=="__main__":
    import random
    dataset_path = "/ssd2/DeepFakes_may/celeb-df_crop_face/"

    for split in {"train", "val", "test"}:
        dataset = CelebDFDataset(dataset_path=dataset_path, split=split, resolution=224)
        print(split)
        print(f"sample keys:", {k: (type(v) if not isinstance(v, torch.Tensor) else v.shape) for k, v in dataset[0].items()})
        print(f"{split}:", dataset.get_label_distribution())
