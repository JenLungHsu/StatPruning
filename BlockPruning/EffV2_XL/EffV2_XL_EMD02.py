...
def EffV2_XL_EMD02():
    ...
    for i in range(31, 2, -1):  # 逆向刪除，避免索引錯亂
        del model.blocks[5][i]

    for i in range(23, 2, -1):  # 逆向刪除，避免索引錯亂
        del model.blocks[4][i]

    for i in range(15, 1, -1):  # 逆向刪除，避免索引錯亂
        del model.blocks[3][i]

    for i in range(7, 2, -1):  # 逆向刪除，避免索引錯亂
...
if __name__ == "__main__":
    ...
    model = EffV2_XL_EMD02().to(device)
    print(model)
