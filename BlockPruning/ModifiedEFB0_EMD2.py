...
class ModifiedEFB0_EMD2(nn.Module):
    def __init__(self, original_model):
        super(ModifiedEFB0_EMD2, self).__init__()
        self.init_block = original_model.features.init_block
        self.stage1 = original_model.features.stage1
        
        # self.adapter_stage1_to_stage2 = nn.Sequential(
        ...
        new_bn = torch.nn.BatchNorm2d(24,
                                      eps=old_bn.eps,
        ...
        self.stage2 = nn.Sequential(
            original_model.features.stage2.unit2
        )

        self.stage3 = original_model.features.stage3
        self.stage4 = original_model.features.stage4
        self.stage5 = nn.Sequential(
            original_model.features.stage5.unit1,
            original_model.features.stage5.unit2,
            original_model.features.stage5.unit3,
    ...
    def forward(self, x):
        x = self.init_block(x)
        x = self.stage1(x)
        # x = self.adapter_stage1_to_stage2(x)
        x = self.stage2(x)
        x = self.stage3(x)
        x = self.stage4(x)
        x = self.stage5(x)
...
if __name__ == "__main__":
    ...
    # 建立新模型，只保留你想要的部分
    modified_model = ModifiedEFB0_EMD2(original_model)
    ...
    with torch.no_grad():
        flops = FlopCountAnalysis(modified_model, torch.randn(1, 3, 224, 224))
        parameters = parameter_count(modified_model)[""]
